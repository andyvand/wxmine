...
// Uses NSHelpManager / showHelp: so the bundled wxmine.help book
...
@interface NSHelpManager (wxMinePrivate)
...
extern "C" void ShowMacHelpBook(const char *anchor)
{
    @autoreleasepool {
        ...
        if ([mgr respondsToSelector:@selector(registerBooksInBundle:error:)]) {
            ...
            if (!registered) {
                NSLog(@"wxmine: registerBooksInBundle failed: %@", err);
            }
        }
        ...
        if (url) {
            ...
            @try { [task launch]; }
            @catch (NSException *e) {
                NSLog(@"wxmine: launching Help Viewer failed: %@", e);
                [[NSApplication sharedApplication] showHelp:nil];
            }
        } else {
            [[NSApplication sharedApplication] showHelp:nil];
